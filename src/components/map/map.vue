<template>
  <div class="container">
            ...
            >
              ...
              <g>
                ...
                <path
                  ...
                  :id="data.id"
                  :title="data.title"
                  :class="[check_active(i) > 0? 'land-active':'land']"
                  :d="data.d"
                  @click="select(data.id,i)"
                >
          ...
          <b-list-group v-for="(data,i) in dataActive" :key="i" hidden>
            ...
            <div class="text-right">
              ...
              <div class>
                ...
                >
                  <template
                    v-slot:cell(province)="data"
                  >{{ data.item.first_name}} {{ data.item.title}}</template>

                  <template v-slot:cell(user)="data">{{ data.item.viewer}}</template>
    ...
    <button @click="reset" hidden>Reset</button>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
        items:[],
        fields: [
        { key: "province", sortable: true, label: "จังหวัด" },
        { key: "user", sortable: true, label: "จํานวน" },
      ],
      hover: false,
      Datas: [],
      user: {
  ...
  created() {
    ...
    axios
      ...
      .then(res => {
        ...
        this.Datas = datas;
        this.items = datas.filter(i=>i.viewer > 0)
